...
class PDFprocessor:
    ...
    def split_pdf(self, output_pdf) -> None:
        ...
        for page_num in range(len(pdf_to_split)):
            new_pdf = fitz.open()
            new_pdf.insert_pdf(pdf_to_split, from_page=page_num, to_page=page_num)
            output_filename = os.path.join(output_pdf, f"page{page_num + 1}.pdf")
    ...
    def pdf_to_png(self, output_pdf) -> None:
        ...
        for page_num in range(len(pdf)): #converts each page of pdf to png (i don't remember why then i need previous def)
            page = pdf[page_num]
            pix = page.get_pixmap(dpi=300) #dpi for better quality
            img = Image.frombytes("RGB", [pix.width, pix.height], pix.samples) 
            output_name = f"{self.name}.png"
            img.save(os.path.join(output_pdf, output_name))
        pdf.close()
...
class ImageProcessor:
    ...
    #converts grayscale image into binary by comparing each pixel intensity against a specified threshold value
    def black_white(self, threshord_arg):
        _, self.image = cv2.threshold(self.image, threshord_arg, 255, cv2.THRESH_BINARY_INV)
        return self
    
    def save_img(self, output_path):
        cv2.imwrite(f'{output_path}/{self.path.name}', self.image)

    #cropping outer border (it can be different in size, so easier just cut it off)
    def crop_to_outer_lines(self):
        kernel = np.ones((15, 15), np.uint8)
        closed = cv2.morphologyEx(self.image, cv2.MORPH_CLOSE, kernel, iterations=3)
        dilated = cv2.dilate(closed, None, iterations=2)
        contours, _ = cv2.findContours(dilated, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE) #finding contours
    ...
    def crop_color_canals(self, output_png):
        ...
        #coordinates of the found lines and calculation of the distance between them
        if unique_lines:
            for i in range(len(unique_lines) - 1):
                ...
                if gap > gap_threshold:
                    cropped_section = self.original[y_top : y_bottom, 0:width] 
                    count += 1
                    cv2.imwrite(f'{output_png}/{self.name}_{count}.png', cropped_section)
            return True
    ...
    def locus_coords_function(self):          
        ...
        data = pytesseract.image_to_data(roi, config = '--psm 11 -c load_system_dawg=0 -c load_freq_dawg=0', output_type=Output.DICT)
        raw_words = []
        ...
        for i, word1 in enumerate(raw_words):
            ...
            x1_loc_name, y1_loc_name, x2_loc_name, h_loc_name = word1['x1_loc_name'], word1['y1_loc_name'], word1['x2_loc_name'], word1['h_loc_name']
            word1['used'] = True
            
            #word stitching for loci consisting of 2 words
            for j, word2 in enumerate(raw_words):
                if word2['used']: continue
                same_line = abs(word1['y1_loc_name'] - word2['y1_loc_name']) < 8
                close_right = 0 <= (word2['x1_loc_name'] - x2_loc_name) < 25 
                blacklist = ['(', ')', '[', ']', '{', '}']

                if same_line and close_right and not [i for i in blacklist if i in word2['text']]:
        ...
        for word in stitched_words:
            ...
            #search for similarities between words found by pytesseract and words in the list
            for l in LOCUS_LIST:
                target_lower = l.lower()
                similarity = fuzz.ratio(target_lower, detected_text_lower)
    ...
    def crop_locus_function(self, locus_coords_name):
        ...
        for locus in locus_coords_name:
            height_crop = int(locus['h_loc_name']/2+locus['y1_loc_name']+30)
            break
        ...
        try:
            for r in range(40, 200, 20): 
                ...
                _, thresh = cv2.threshold(roi, r, 255, cv2.THRESH_BINARY_INV)
                kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (1, 30)) #matrix for vertical lines
                detected_lines = cv2.morphologyEx(thresh, cv2.MORPH_OPEN, kernel) #transformation of picture so only 
                                                                                   #vertical lines is visible
                lines = cv2.HoughLinesP(detected_lines, 1, np.pi/180, threshold=20, #detection of vertical lines
                                            minLineLength=25, maxLineGap=5) 
                
                for item in locus_coords_name:
                    x1_loc_name, w_loc_name, h_loc_name = item['x1_loc_name'], item['w_loc_name'], item['y1_loc_name'] + item['h_loc_name']
                    ...
                    if lines is not None:
                        xs = [line[0][0] for line in lines] + [line[0][2] for line in lines] 
                        ...
                        if right_candidates:
                            ...
                            if right_border < item['x1_loc_name'] + item['w_loc_name']:
                                right_border = item['x1_loc_name'] + item['w_loc_name']

                    borders_dict = {'file_name': self.name, 'name': item['name'], 'x1_locus': int(left_border), 'x2_locus': int(right_border), 'h_locus': int(h_loc_name), 'middle': middle} 
                    raw_locus_coords.append(borders_dict)
                
                sorted_raw_locus_coords = sorted(raw_locus_coords, key=lambda x: x['x1_locus'])
                if not sorted_raw_locus_coords:
                    break

                current_iteration_valid = []

                #iteration to select the optimal threshold parameter for the binary image so that the loci 
                #do not overlap with each other by taking pairs of loci and compare their coordinates of right and left borders
                for i in range(len(sorted_raw_locus_coords) - 1):
                    if sorted_raw_locus_coords[i]['x2_locus'] >= sorted_raw_locus_coords[i+1]['x1_locus']:
                        break
                    else:

                        if sorted_raw_locus_coords[i]['middle']-sorted_raw_locus_coords[i]['x1_locus']+100 >= sorted_raw_locus_coords[i]['x2_locus']-sorted_raw_locus_coords[i]['middle']:
                            
                            if sorted_raw_locus_coords[i]['middle']-sorted_raw_locus_coords[i]['x1_locus'] <= sorted_raw_locus_coords[i]['x2_locus']-sorted_raw_locus_coords[i]['middle']+100:
                    ...
                    if sorted_raw_locus_coords[-1]['x1_locus'] >= sorted_raw_locus_coords[-2]['x2_locus']:

                        if sorted_raw_locus_coords[-1]['middle']-sorted_raw_locus_coords[-1]['x1_locus']+100 >= sorted_raw_locus_coords[-1]['x2_locus']-sorted_raw_locus_coords[-1]['middle']:
                            
                            if sorted_raw_locus_coords[-1]['middle']-sorted_raw_locus_coords[-1]['x1_locus'] <= sorted_raw_locus_coords[-1]['x2_locus']-sorted_raw_locus_coords[-1]['middle']+100:
                                
                                if sorted_raw_locus_coords[-1]['name'] not in seen:
                                    current_iteration_valid.append(sorted_raw_locus_coords[-1])
...
class allele_data: #input - color canal
    ...
    def black_white(self, threshord_arg):
        _, self.image = cv2.threshold(self.image, threshord_arg, 255, cv2.THRESH_BINARY)
        return self
    ...
    def allele_roi_function(self, list_of_borders):
        ...
        #searching by every pixel in y-coordinate (the same as for color canals)
        for y in range(height): 
            row = self.image[y, :]
        ...
        if unique_lines:
            for i in range(len(unique_lines) - 1):
                if unique_lines[i+1] < height-10:
                    ...
                    if gap > min_gap_threshold and gap < max_gap_threshold:
                        y_locus_true = y_bottom

        new_list_of_borders = []
        if y_locus_true is not None:
            for l in list_of_borders:
                if l['file_name'] in self.name and y_locus_true:
    ...
    def allele_contours(self, list_of_borders):
        ...
        for l in list_of_borders:
            if l['file_name'] in self.name:
                list_lines = []
                locus_area = self.image[l['y_locus']:, l['x1_locus']:l['x2_locus']]

                h_locus = locus_area.shape[0]

                #finding all of contours in the image
                contours, _ = cv2.findContours(
                    locus_area, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
                    ) 
                ...
                for _, cunt in enumerate(contours):
                    ...
                    for eps in np.linspace(0.001, 0.05, 10): 
                        ...
                        if len(approx) == 4: #if there's 4 points - it's a square
                            ...
                            if h_bbox<40 or h_bbox>300 or w_bbox>300 or h_bbox/2 > \
                                    w_bbox or h_bbox+y_bbox == h_locus: 
                                break
                            else:
                    ...
                    for box in filtered_boxes:
                        ...
                        if data and box[0] <= 1:
                            left_border, right_border = self.change_border(
                                'left', l['x1_locus'] + box[0], l['y_locus']+box[1], 
                                box[2], box[3]
                                )
                        ...
                        #for right side
                        right_side_calc = l['x2_locus'] - l['x1_locus'] - box[0] - \
                            box[2]

                        if data and right_side_calc <= 1:
                            left_border, right_border = self.change_border(
                                'right', l['x1_locus'] + box[0], l['y_locus']+box[1], 
                                box[2], box[3]
                                )
    ...
    def change_border(
            self, border, x_change_bbox, y_change_bbox, w_change_bbox, h_change_bbox
            ):
        
        width = self.original.shape[1]

        _, tresh = cv2.threshold(self.original, 150, 255, cv2.THRESH_BINARY_INV)
        roi_right = tresh[y_change_bbox:h_change_bbox+y_change_bbox, 0:width]
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (1, 30))
        detected_lines = cv2.morphologyEx(roi_right, cv2.MORPH_OPEN, kernel)
        ...
        if lines is not None:
            ...
            if border == 'left':
                left_candidates = [
                    cx for cx in xs if cx < (left_border + w_change_bbox/2)
                    ]
            ...
            if border == 'right':
                right_candidates = [
                    cx for cx in xs if cx > (right_border - w_change_bbox/2)
                    ]
...
def main():
    ...
    for pdf in pdf_dir.glob('*.pdf'):
        ...
        count += 1
        count_files = sum(1 for pdf in pdf_dir.iterdir() if pdf.is_file())

        try:
            print(f'({count}/{count_files}) Processing file {pdf.name}...')
        ...
        for img in img_dir.glob('*.png'):
            try:
                ...
                print(f'\033[31mProblems with cropping color canals: {e}.\033[0m')
                break
            ...
            if locus_coords:
                try:
                    ...
                    for r in range (50, 190, 20):
                        new_locus_coords = allele_data(crop).black_white(r)\
                    ...
                    df = allele_data(crop).black_white(180)\
        ...
        try:
            ...
            no_dopplers = full_df.loc[:, ~full_df.columns.duplicated()] 
            no_dopplers.to_excel(f"{df_dir}/{pdf.name}.xlsx")
            print(f'\033[32mFile {pdf.name} was processed in {\
                execution_time:.2f} seconds.\033[0m')

        except Exception as e:
            print(f'\033[31mThere is no dataframe for {pdf.name}. Error: {e}\033[0m')
